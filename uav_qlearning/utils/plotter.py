...
def plot_training_stats(episode_rewards, episode_lengths, successful_episodes, communication_qualities, window=100):
    ...
    ax2.plot(length_avg, label=f'Moving Average ({window} episodes)', color='red', linewidth=2)
    ...
    # Plot success rate
    successes = np.array(successful_episodes)
    episodes = np.arange(1, len(successes) + 1)
    success_rate = (successes / episodes) * 100
    
    ax3.plot(episodes, success_rate, label='Success Rate', color='purple', linewidth=2)
