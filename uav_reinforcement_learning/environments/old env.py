# [file name]: uav_qlearning/environments/uav_env.py
# [file content begin]
...
class UAVEnv(gym.Env):
    ...
    def __init__(self, grid_size=15, render_mode=None):
        ...
        # Define observation space: UAV position + distances to users + step counter
    ...
    def _get_observation(self):
        """Get the current observation including distances to users and step information"""
        # Normalize position to [0, 1] range
        uav_x = self.current_pos[0] / (self.grid_size - 1)
        uav_y = self.current_pos[1] / (self.grid_size - 1)
        
        # Calculate distances to users (normalized)
    ...
    def _calculate_reward(self):
        ...
        # If reached goal early, small penalty
        if np.array_equal(self.current_pos, self.goal_pos) and self.steps < self.max_steps:
            return -5, False
        
        # Simple reward: communication quality minus small step penalty
        comm_quality = self._get_communication_quality()
        step_penalty = -0.05  # Very small step penalty
        
        # Small bonus for exploring new areas
        exploration_bonus = 0.1 if len(self.trajectory) == len(set(map(tuple, self.trajectory))) else 0
        
        total_reward = comm_quality + step_penalty + exploration_bonus
        
        return total_reward, False
    
    def _calculate_final_reward(self):
        """Calculate final reward at the end of episode"""
        # Check if we're at the goal
        at_goal = np.array_equal(self.current_pos, self.goal_pos)
        
        # Calculate average communication quality throughout the episode
        if len(self.trajectory) > 0:
            total_comm = sum(self._get_communication_quality_for_pos(pos) for pos in self.trajectory)
            avg_comm = total_comm / len(self.trajectory)
        else:
            avg_comm = 0
        
        # Final reward components
        goal_bonus = 10 if at_goal else 0
        comm_bonus = avg_comm * 5
        
        # Check if we visited user areas
        visited_users = False
        for pos in self.trajectory:
            user1_dist = np.linalg.norm(pos - np.array(self.users[0]))
            user2_dist = np.linalg.norm(pos - np.array(self.users[1]))
            if user1_dist < 8 or user2_dist < 8:
                visited_users = True
                break
        
        exploration_bonus = 5 if visited_users else 0
        
        return goal_bonus + comm_bonus + exploration_bonus
    
    def _get_communication_quality_for_pos(self, pos):
        """Calculate communication quality for a given position"""
        x, y = pos
        
        # Base quality from NLOS conditions
        ...
        user1_signal = 1.0 / (1.0 + user1_dist / 5)
        user2_signal = 1.0 / (1.0 + user2_dist / 5)
        
        # Combined signal quality
        distance_quality = (user1_signal + user2_signal) / 2
        
        # Combine base quality and distance quality
        combined_quality = (base_quality * 0.3) + (distance_quality * 0.7)
        
        return min(1.0, combined_quality)
    
    def _get_communication_quality(self):
        """Calculate communication quality for current position"""
    ...
    def render(self):
        ...
        # Make goal flash if we're at the goal
        at_goal = np.array_equal(self.current_pos, self.goal_pos)
        if at_goal:
            # Flash between green and yellow
            flash = (pygame.time.get_ticks() // 500) % 2
            goal_color = (0, 255, 0) if flash else (255, 255, 0)  # Green/Yellow flash
        else:
            goal_color = (0, 255, 0)  # Normal green
        
        pygame.draw.rect(self.screen, goal_color, goal_rect)  # Green background
        ...
        # Make UAV flash red if at goal
        if at_goal:
            flash = (pygame.time.get_ticks() // 250) % 2
            uav_color = (255, 0, 0) if flash else (255, 165, 0)  # Red/Orange flash
        else:
            uav_color = (255, 165, 0)  # Normal orange
        
        pygame.draw.circle(self.screen, uav_color, (uav_x, uav_y), 8)  # Orange UAV
        
        # Display information
        user1_dist = np.linalg.norm(self.current_pos - np.array(self.users[0]))
        user2_dist = np.linalg.norm(self.current_pos - np.array(self.users[1]))
        ...
        info_text = [
            ...
            f"Comm Quality: {self._get_communication_quality():.2f}",
            f"Dist to UE1: {user1_dist:.1f}, UE2: {user2_dist:.1f}",
            f"Dist to Goal: {distance_to_goal:.1f}",
            f"At Goal: {'YES' if at_goal else 'NO'}"
        ]
        ...
        pygame.display.flip()
        # Remove frame rate limit for maximum speed
    
    def get_state_index(self, state):
        """Convert continuous state to discrete index for Q-table - for agent use"""
        # Extract UAV position from state vector [uav_x, uav_y, ...]
